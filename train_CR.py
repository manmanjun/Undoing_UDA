...
class CrossEntropyLoss2dPixelWiseWeighted(nn.Module):
    def __init__(self, weight=None, ignore_index=250, reduction='none'):
        ...
        # GTA5 class frequence 
        pixel_fre = [24353, 23847, 24724, 19311, 11000, 24153, 10641, 5725, 24553, 20667, 24517, 12910, 1140, 20311, 11555, 2518, 299, 1204, 258]
        pixel_fre = torch.Tensor(pixel_fre).cuda()
        pixel_fre = torch.log(pixel_fre/torch.sum(pixel_fre))
        self.pixel_fre = pixel_fre.reshape(1,19,1,1)
        
        #estimate city class frequence by  deeplabv2
        pixel_fre_tgt = [2531, 1309, 2154, 805, 739, 2016, 1180, 1127, 2293, 833, 239, 1515, 789, 2171, 431, 173, 107, 593, 1158] 
    
        # estimate city class frequence by deeplabv3+
        # pixel_fre_tgt = [2819, 813, 2203, 391, 763, 1608, 1055, 1131, 2277, 912, 260, 1396, 944, 2004, 256, 266, 138, 631, 1075] 

        pixel_fre_tgt = torch.Tensor(pixel_fre_tgt).cuda()
        pixel_fre_tgt = torch.log(pixel_fre_tgt/torch.sum(pixel_fre_tgt))
        self.pixel_fre_tgt = pixel_fre_tgt.reshape(1,19,1,1)
    def forward(self, output, target):
        loss = self.CE(output + self.pixel_fre - self.pixel_fre_tgt, target)
...
def train(cfg, local_rank, distributed):
    ...
    labeled_loss = CrossEntropyLoss2dPixelWiseWeighted(ignore_index=255).cuda()
...
#       torch.distributed.barrier()
        ...
        loss_seg = labeled_loss(src_pred, src_label)
        loss_seg.backward()
        optimizer_cls.step()
